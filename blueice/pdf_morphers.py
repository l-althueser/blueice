...
class GridInterpolator(Morpher):
    ...
    def make_interpolator(self, f, extra_dims, anchor_models):
        ...
        for anchor_grid_index, _zs in self._anchor_grid_iterator():
            ...
            anchor_scores[tuple(anchor_grid_index + [slice(None)] * len(extra_dims))] = f(anchor_models[tuple(_zs)])
    ...
    def _anchor_grid_iterator(self):
        ...
        while not it.finished:
            ...
            yield anchor_grid_index, tuple(self.anchor_z_grid[tuple(anchor_grid_index + [slice(None)])])
            it.iternext()
